...
test('the control model notifies subscribers until they unsubscribe', () => {
    const model = new ControlModel(createState(), () => ({sources: {aux: {enabled: true}}, routesKnown: true}))
    assert.deepEqual(model.audio, {sources: {aux: {enabled: true}}, routesKnown: true})
