...
test('the current mode is read back from the audio manager, not remembered', () => {
  ...
  const stream = tile.render(testContext()).buffer
  const aux = tile.render(testContext(undefined, { sources: { aux: true } })).buffer
  const usb = tile.render(testContext(undefined, { sources: { usb: true } })).buffer
  ...
  assert.deepEqual(tile.render(testContext(undefined, { sources: { aux: true } })).buffer, aux)
})
